...
while true # The script will repeat below until CTRL-C is pressed
	do
		clear
		HOSTSDOWN=`cat $WDEXPORTFILE | awk -F":" '{print $1}' $2` #Extract hostsdown number from export file generated by watchdog1337
		HOSTS=`cat $WDEXPORTFILE | awk -F":" '{print $2}' $2` #Excract total number of hosts from export file...
			...
			if [ $HOSTSDOWN != 0 ]; then # If the number of offline hosts are higher than 0, then:
				if [[ $TEMPC -gt 5 ]]; then
					...
					echo "Possible false positive, waiting (Threshold $TEMPC of 6)"
					sleep 5
					((TEMPC++))
				fi
